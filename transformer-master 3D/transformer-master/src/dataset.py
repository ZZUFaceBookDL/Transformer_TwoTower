...
class SingleDimensionalDataset(Dataset):
    def __init__(self, filepath):
        super(SingleDimensionalDataset, self).__init__()
        xy = np.loadtxt(filepath, delimiter='\t', dtype=np.float32)
        # 此时取出来的数据是一个(N,M)的元组，N是行，M是列，xy.shape[0]可以把N取出来，也就是一共有多少行，有多少条数据
        self.len = xy.shape[0]
        # self.x_data = torch.from_numpy(xy[:, 1:])
        self.x_data = xy[:, 1:]
        self.column = self.x_data.shape[1]
        self.data_length_p = self.x_data.shape[0]
        self.y_data = xy[:, 0]
        self.feature = len(set(self.y_data))
